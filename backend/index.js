...
app.get("/status", async (req, res) => {
  try {
    const assigned = await AssignedComplaint.find().select("complaintId");

    const assignedIds = assigned.map(a => a.complaintId);
...
app.post("/assignedComplaints", async (req, res) => {
  try {
    ...
    const exists = await AssignedComplaint.findOne({ complaintId });
    ...
    const assignedComplaint = await AssignedComplaint.create({
      ...req.body,
      status: "assigned"
    });

    res.status(201).json(assignedComplaint);
...
app.get("/allcomplaints/:agentId", async (req, res) => {
  try {
    ...
    //recent edited const complaints = await AssignedComplaint.find({ agentId: agentId });
...
app.put("/complaint/:complaintId", async (req, res) => {
  try {
    ...
    const updatedAssigned = await AssignedComplaint.findOneAndUpdate(
      { complaintId },
      { status },
      { new: true }
    );
